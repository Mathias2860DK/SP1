...
class Game
{
  ...
  private void updateEnemies()
  {
    for(int i = 0; i < enemies.length; ++i)
    {
      ...
      if(rnd.nextInt(3) < 2)
      {
        if((int)dist(player.getX(),player.getY(),enemies[i].getX(),enemies[i].getY()) < dist(player2.getX(),player2.getY(),enemies[i].getX(),enemies[i].getY())){
        //We follow player 1
  ...
  {
    //Check enemy collisions - player1
    for(int i = 0; i < enemies.length; ++i)
    {
      if(enemies[i].getX() == player.getX() && enemies[i].getY() == player.getY())
      {
        //We have a collision - emenies og player1
        --playerLife;
      }
    }
     ...
     for(int i = 0; i < food.length; ++i)
    {
      ...
      {
        //We have a collision med food og player
        ++playerLife;
      }
    }
      //Check for food collisions 
     for(int i = 0; i < food.length; ++i)
    {
      ...
      //Life kan aldrig komme over maxLife. værdi sat øverest.
      ...
      if (playerLife2 > maxLife){
       ...
       //Hvis spiller 1 rammer food, spawner food et random sted
      if(food[i].getX() == player.getX() && food[i].getY() == player.getY()){
        food[i] = new Dot ((int)random(width-1), (int)random(height-1), width-1, height-1);

      }
      //Hvis spiller 2 rammer food, spawner food et random sted
      if(food[i].getX() == player2.getX() && food[i].getY() == player2.getY()){
        food[i] = new Dot ((int)random(width-1), (int)random(height-1), width-1, height-1);
